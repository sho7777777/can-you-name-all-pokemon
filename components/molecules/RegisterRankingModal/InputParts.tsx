...
export const InputParts: FC<InputPartsProps> = (props) => {

  const { registerRanking, userName, setUserName, userNameValidation, setUserNameValidation } = props
  ...
  const onEnter = (e: any) => {
    ...
    if (e.key === "Enter" && userName.length > 0 && userName.length < usrNameMaxLength) {
      registerRanking()
    }
  }
  ...
  return (
    <div>
      ...
      {userNameValidation.isUserNameUndefined && <p className="text-red-600">なまえを入力してね</p>}
      {userNameValidation.isUserNameExceedsMaxLength && <p className="text-red-600">なまえは{usrNameMaxLength}文字以内にしてね</p>}
      {userNameValidation.isCharacterInvalid && <p className="text-red-600">その文字は使わないでね</p>}
    </div>
  )
}
